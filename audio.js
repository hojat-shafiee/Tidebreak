...
class AudioManager {
  ...
  playCollectSound(itemType) {
    ...
    if (itemType === ITEM_TYPES.STAR) {
      ...
      // Magnetic pulse
      this.playSound(300, 0.2, "sawtooth", 0.4);
      setTimeout(() => this.playSound(400, 0.2, "sawtooth", 0.4), 50);
    } else if (itemType === ITEM_TYPES.SLOWMO) {
      // Time warp
      this.playSoundSequence([800, 600], 0.2, 0.1, 0.4);
    } else if (itemType === ITEM_TYPES.SHIELD) {
      // Shield activate
      this.playSound(1200, 0.3, "sine", 0.5);
    }
  }
